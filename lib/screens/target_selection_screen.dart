  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton.extended(
        ...
        label: Text('Add Custom Interval', style: GoogleFonts.roboto(color: Colors.white, fontWeight: FontWeight.bold)),
      ),
      body: ListView(
        padding: const EdgeInsets.only(bottom: 100),
        children: [
          _SectionHeader(label: 'Official Milestones'),
          ...officialTests.where((t) => t.distance != null || (t.speedUnit == (isMetric ? SpeedUnit.kmh : SpeedUnit.mph))).map((target) {
            return _TargetTile(
              target: target,
          ...
          if (dragy.customTargets.isNotEmpty) ...[
            _SectionHeader(label: 'Custom Intervals'),
            ...dragy.customTargets.map((target) {
              return _TargetTile(
                target: target,
                isEnabled: dragy.enabledTargets.contains(target.id),
                onChanged: (val) => dragy.toggleTargetEnabled(target.id, val),
    ...
    return Container(
      ...
      child: SwitchListTile(
        ...
        subtitle: target.distance != null 
          ? Text('Distance target', style: GoogleFonts.roboto(color: Colors.white38, fontSize: 12))
          : Text('Speed interval', style: GoogleFonts.roboto(color: Colors.white38, fontSize: 12)),
        value: isEnabled,
        onChanged: onChanged,
...
class _AddCustomTargetDialogState extends State<_AddCustomTargetDialog> {
  late bool isMetric;
  final TextEditingController _startController = TextEditingController(text: '100');
  final TextEditingController _endController = TextEditingController(text: '200');

  @override
  void initState() {
    super.initState();
    isMetric = widget.dragy.isMetric;
    if (!isMetric) {
      _startController.text = '60';
      _endController.text = '130';
    }
  }
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      ...
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(14)),
      title: Text('New Custom Interval', style: GoogleFonts.roboto(color: Colors.white)),
      content: Column(
        ...
        children: [
          ...
          Row(
            ...
            children: [
              Text('mph', style: GoogleFonts.roboto(color: Colors.white54)),
              Switch(
                value: isMetric,
                onChanged: (v) => setState(() => isMetric = v),
                activeColor: const Color(0xFF42A5F5),
              ),
              Text('km/h', style: GoogleFonts.roboto(color: Colors.white54)),
            ],
          ),
        ],
      ),
      actions: [
        ...
        TextButton(
          onPressed: () {
            ...
            if (start != null && end != null && end > start) {
              ...
              final target = RaceTarget(
                ...
                startSpeed: isMetric ? start : start * 1.609344, // always store in km/h
                endSpeed: isMetric ? end : end * 1.609344, // always store in km/h
                speedUnit: unit,
                isOfficial: false,
              );
          ...
          child: Text('Add', style: GoogleFonts.roboto(color: const Color(0xFF42A5F5))),
        ),
      ],
    );
  }
