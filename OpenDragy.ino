...
void gpsReadTask(void *parameter) {
  ...
  while (1) {
    while (UART.available() > 0) {
      ...
      switch (state) {
        case 0:
          if (c == 0xB5) { buffer[0] = c; state = 1; idx = 1; }
          break;
        case 1:
          if (c == 0x62) { buffer[1] = c; state = 2; idx = 2; }
          else { state = 0; }
          break;
        case 2: // Class
          buffer[idx++] = c; state = 3;
          break;
        case 3: // ID
          buffer[idx++] = c; state = 4;
          break;
        case 4: // Length LSB
          buffer[idx++] = c; expectedLen = c; state = 5;
          break;
        case 5: // Length MSB
          buffer[idx++] = c; expectedLen |= (c << 8);
          // NAV-PVT payload is usually 92 or 100 bytes. Reject lengths that are too large.
          if (expectedLen > 110) { 
            state = 0;
          } else {
            state = 6;
        ...
        case 6: // Payload
          buffer[idx++] = c;
          if (idx == expectedLen + 6) { // header(2) + cls(1) + id(1) + len(2) + payload
            state = 7;
          }
          break;
        case 7: // Checksum A
          buffer[idx++] = c; state = 8;
          break;
        case 8: // Checksum B
          buffer[idx++] = c;
          ...
          if (ck_a == buffer[idx-2] && ck_b == buffer[idx-1]) {
            ...
            if (buffer[2] == 0x01 && buffer[3] == 0x07) {
              // Send to FIFO queue, do not block if full (we drop the oldest or drop this one)
  ...
  while (1) {
    if (xQueueReceive(gpsQueue, buffer, portMAX_DELAY) == pdTRUE) {
      if (deviceConnected && pTxCharacteristic) {
        ...
        uint16_t totalLen = len + 8; // Include header, class, id, length, payload, checksum
...
void setup() {
  ...
  if (!imuReady) {
    Serial.println("[CRITICAL ERROR] BMI160 hardware not responding. Running in GPS-only mode.");
  }
  ...
  xTaskCreatePinnedToCore(bleNotifyTask, "BLE_Notify", 4096, NULL, 1, NULL, 0); // Run on core 0 with BLE stack
